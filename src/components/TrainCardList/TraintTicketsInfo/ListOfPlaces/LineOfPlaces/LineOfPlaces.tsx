import {findMinimumPrice} from "../../../../../utils/findMinimumPrice";
import {useState} from "react";
import styles from "./LineOfPlaces.module.css";
import {PopOver} from "../../../../../ui/PopOver/PopOver";

export const LineOfPlaces = ({item}: {item: any}) => {
  const [showPopOver, setShowPopOver] = useState(false);
  const [dataTypeLineOfPlaces, setDataTypeLineOfPlaces] = useState("");

  const {first, second, third, fourth} = item.departure.available_seats_info;
  const {
    first: firstPrice,
    second: secondPrice,
    third: thirdPrice,
  ...
  const handleMouseEnter = (type: any) => {
    setShowPopOver(true);
    setDataTypeLineOfPlaces(type);
  };

  const handleMouseLeave = () => {
    setShowPopOver(false);
    setDataTypeLineOfPlaces("");
  };
    ...
    <div>
      {fourth && (
        <div className={styles.popover_wrapper}>
          ...
          >
            <div className={styles.type_places}>Сидячий</div>
            <div className={styles.number_seats}>{[fourth]}</div>
            <span className={styles.from}> от </span>
            <div className={styles.wrapper_cost}>
              <span className={styles.minimum_cost}>
                {findMinimumPrice(fourthPrice)}
              </span>
              <svg
                width="16"
                height="19"
          ...
          {showPopOver && dataTypeLineOfPlaces === "fourth" && (
            <PopOver
              priceInfo={item?.departure?.price_info}
              dataType={dataTypeLineOfPlaces}
            />
          )}
        </div>
      )}
      {third && (
        <div className={styles.popover_wrapper}>
          <div
            ...
            onMouseLeave={handleMouseLeave}
          >
            <div className={styles.type_places}>Плацкарт</div>
            <span className={styles.number_seats}>{[third]}</span>
            <span className={styles.from}> от </span>
            <div className={styles.wrapper_cost}>
              <span className={styles.minimum_cost}>
                {findMinimumPrice(thirdPrice)}
              </span>
              <svg
                width="16"
                height="19"
      ...
      {second && (
        <div className={styles.popover_wrapper}>
          <div
            ...
            onMouseLeave={handleMouseLeave}
          >
            <div className={styles.type_places}>Купе</div>
            <span className={styles.number_seats}>{[second]}</span>
            <span className={styles.from}> от </span>
            <div className={styles.wrapper_cost}>
              <span className={styles.minimum_cost}>
                {findMinimumPrice(secondPrice)}
              </span>
              <svg
                width="16"
                height="19"
      ...
      {first && (
        <div className={styles.popover_wrapper}>
          <div
            ...
            onMouseLeave={handleMouseLeave}
          >
            <div className={styles.type_places}>Люкс</div>
            <span className={styles.number_seats}>{[first]}</span>
            <span className={styles.from}> от </span>
            <div className={styles.wrapper_cost}>
              <span className={styles.minimum_cost}>
                {findMinimumPrice(firstPrice)}
              </span>
              <svg
                width="16"
                height="19"
