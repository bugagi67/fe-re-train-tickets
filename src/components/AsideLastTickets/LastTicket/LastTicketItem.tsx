...
import styles from "./LastTicketItem.module.css";

export const LastTicketItem = ({item}: {item: any}) => {
  return (
    <div className={styles.last_ticket_item}>
      <div className={styles.wrapper_last_ticket}>
        ...
        <div className={styles.arrival_info}>
          <div className={styles.arrival_city}>{item?.departure?.to?.city?.name}</div>
          <div className={styles.arrival_railway_station}>
            {item?.departure?.to?.railway_station_name} вокзал
          </div>
        </div>
      </div>
      <div className={styles.footer_last_ticket}>
        <IconConveniences item={item}/>
        <div className={styles.last_ticket_price}>
          <span className={styles.text}>от</span> <span>{item?.min_price}</span>
          <svg
            width="16"
            height="19"
            viewBox="0 0 16 19"
