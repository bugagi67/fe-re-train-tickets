...
export const DetailedRoute = () => {
  const { routeData, data } = useSelector( state => state.selectedSlice );
  if ( !data || !data[ 0 ] ) return <div>Данные о поезде отсутствуют</div>
  const { coach } = data[ 0 ];
  ...
  return (
    <div className={ styles.wrapper }>
      <div className={ styles.first_wrapper }>
        ...
        <div>
          <div className={ styles.bold_text }>{ coach.name }</div>
