...
import { formatedTime } from "../../../../utils/formatedTime.ts";
import { calculateTravelTime } from "../../../../utils/calculateTravelTime.tsx";
import type { RootState } from "../../../../redux/store/store.ts";
import styles from "./DetailedRoute.module.css"

export const DetailedRoute = () => {
  const { routeData, currentCarriage } = useSelector((state: RootState) => state.selectedSlice);

  if (!currentCarriage || !routeData) return <div>Данные о поезде отсутствуют</div>
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.first_wrapper}>
        ...
        <div>
          <div className={styles.bold_text}>{currentCarriage?.coach.name}</div>
          <div className={styles.city_name}>{routeData.departure.from.city.name} →</div>
          <div className={styles.city_name}>{routeData.departure.to.city.name}</div>
        </div>
      </div>
      <div>
        <div className={styles.bold_text}>
          {formatedTime(routeData.departure.from.datetime)}
        </div>
        <div className={styles.city_name}>{routeData.departure.from.city.name}</div>
        <div className={styles.station_name}>{routeData.departure.from.railway_station_name}</div>
      </div>
      <div>
        <svg
          width="30"
      ...
      <div>
        <div className={styles.bold_text}>{formatedTime(routeData?.departure.to.datetime)}</div>
        <div className={styles.city_name}>{routeData?.departure.to.city.name}</div>
        <div className={styles.station_name}>{routeData?.departure.to.railway_station_name}</div>
      </div>
      <div className={styles.last_wrapper}>
        ...
        {calculateTravelTime(
          routeData?.departure.from.datetime,
          routeData?.departure.to.datetime, "select"
        )}
      </div>
    </div>
  )
